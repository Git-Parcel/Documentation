...
- 本模组提供少数不同版本间的迁移功能
- 本模组不保证正确迁移其他模组或数据包中的内容
- 这不是本模组的核心功能，在开发的初期阶段，可能仅提供少数重要版本间的迁移

例如，本模组的某个版本支持下面这两种迁移：

- `1.20.1` -> `1.21`
- `1.21` -> `1.21.11`

假设当前游戏版本是`1.21.11`，要加载的**区**的MC版本是`1.20.1`，那么本模组会找到这么一条迁移路径：`1.20.1` -> `1.21` -> `1.21.11`
...
- 保存为文本格式时，如果需要彻底重新处理整个区，会不会太费时间
- 加载时，即使只修改了一个方块，也需要重新加载整个区吗
